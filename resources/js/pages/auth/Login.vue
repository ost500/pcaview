    ...
    <AuthLayout title="로그인" description="PCAview에 오신 것을 환영합니다" :class="{ 'pt-0': hideHeader }">
        ...
        <Form v-bind="AuthenticatedSessionController.store.form()" :reset-on-success="['password']" v-slot="{ errors, processing }" class="space-y-4 sm:space-y-5">
            <div class="space-y-3 sm:space-y-4">
                <div class="space-y-1.5 sm:space-y-2">
                    ...
                    <Input
                        ...
                        class="h-10 rounded-lg border-gray-300 px-3 text-sm transition-all focus:border-blue-500 focus:ring-2 focus:ring-blue-200 sm:h-12 sm:px-4 sm:text-base"
                    />
                ...
                <div class="space-y-1.5 sm:space-y-2">
                    <div class="flex items-center justify-between">
                        <Label for="password" class="text-xs font-medium text-gray-700 sm:text-sm">비밀번호</Label>
                        <TextLink
                            v-if="canResetPassword"
                            :href="request()"
                            class="text-[10px] text-blue-600 hover:text-blue-700 hover:underline sm:text-xs"
                            :tabindex="5"
                        >
                            비밀번호 찾기
                        </TextLink>
                ...
                <div class="flex items-center">
                    <Label for="remember" class="flex cursor-pointer items-center space-x-2">
                        ...
                        <span class="text-xs text-gray-600 sm:text-sm">로그인 상태 유지</span>
                    </Label>
                </div>

                <Button
                    type="submit"
                    class="mt-4 h-10 w-full rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 text-sm font-semibold text-white transition-all hover:from-blue-700 hover:to-purple-700 hover:shadow-lg disabled:opacity-50 sm:mt-6 sm:h-12 sm:text-base"
                    :tabindex="4"
                    :disabled="processing"
                >
                    <LoaderCircle v-if="processing" class="mr-2 h-4 w-4 animate-spin sm:h-5 sm:w-5" />
            ...
            <div class="relative">
                ...
                <div class="relative flex justify-center text-[10px] sm:text-xs">
            ...
            <button
                ...
                type="button"
                class="flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-[#FEE500] text-sm font-semibold text-[#000000] opacity-85 transition-all hover:opacity-100 hover:shadow-lg sm:h-12 sm:text-base"
                :tabindex="6"
            >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" class="sm:h-5 sm:w-5">
                    <path
